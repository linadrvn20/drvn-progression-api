...
# ---------------------------------------------------------------------
PAYMENT_REQUIRED = True           # payments are live
PRICE_USDC = "0.02"               # price per call
YOUR_WALLET_ADDRESS = "0xe7b7a8F420c3f18E04D4E7E7Ad62233640Cc1FC5"

# TESTNET MODE: uses the free x402.org facilitator (no API key needed)
# and Base Sepolia testnet - nothing here costs real money. This is a
# temporary way to confirm the payment loop works end-to-end while the
# CDP mainnet facilitator authorization issue gets sorted separately.
# Flip USE_TESTNET to False and fill in CDP keys to go back to mainnet.
USE_TESTNET = True
NETWORK = "eip155:84532" if USE_TESTNET else "eip155:8453"
FACILITATOR_URL = "https://x402.org/facilitator" if USE_TESTNET else None
...
def _wire_up_real_payments():
    ...
    if USE_TESTNET:
        # Free facilitator, no auth needed - testnet only.
        facilitator_client = HTTPFacilitatorClientSync({"url": FACILITATOR_URL})
    else:
...
if __name__ == "__main__":
    if PAYMENT_REQUIRED:
        if not USE_TESTNET and not (CDP_API_KEY_ID and CDP_API_KEY_SECRET):
            raise RuntimeError(
